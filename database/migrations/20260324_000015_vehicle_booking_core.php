    ...
    'up' => function (PDO $pdo): void {
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS drivers (
                ...
                CONSTRAINT fk_drivers_branch FOREIGN KEY (branch_id) REFERENCES branches(id) ON DELETE SET NULL ON UPDATE CASCADE,
                CONSTRAINT fk_drivers_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE SET NULL ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS pricing_rules (
                ...
                CONSTRAINT fk_pricing_branch FOREIGN KEY (branch_id) REFERENCES branches(id) ON DELETE SET NULL ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS vehicle_bookings (
                ...
                CONSTRAINT fk_vehicle_bookings_branch FOREIGN KEY (branch_id) REFERENCES branches(id) ON DELETE SET NULL ON UPDATE CASCADE,
                CONSTRAINT fk_vehicle_bookings_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE SET NULL ON UPDATE CASCADE,
                CONSTRAINT fk_vehicle_bookings_driver FOREIGN KEY (driver_id) REFERENCES drivers(id) ON DELETE SET NULL ON UPDATE CASCADE,
                CONSTRAINT fk_vehicle_bookings_coupon FOREIGN KEY (coupon_id) REFERENCES booking_coupons(id) ON DELETE SET NULL ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS booking_status_logs (
                ...
                CONSTRAINT fk_booking_status_logs_booking FOREIGN KEY (booking_id) REFERENCES vehicle_bookings(id) ON DELETE CASCADE ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS vehicle_availability (
                ...
                CONSTRAINT fk_vehicle_availability_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE CASCADE ON UPDATE CASCADE,
                CONSTRAINT fk_vehicle_availability_booking FOREIGN KEY (booking_id) REFERENCES vehicle_bookings(id) ON DELETE SET NULL ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS booking_otp_verifications (
                ...
                CONSTRAINT fk_booking_otp_booking FOREIGN KEY (booking_id) REFERENCES vehicle_bookings(id) ON DELETE CASCADE ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS vehicle_maintenance_logs (
                ...
                CONSTRAINT fk_vehicle_maintenance_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE CASCADE ON UPDATE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci'
        );
        ...
        $couponCount = (int) ($pdo->query('SELECT COUNT(*) AS c FROM booking_coupons')->fetch()['c'] ?? 0);
        if ($couponCount === 0) {
            $pdo->exec("INSERT INTO booking_coupons (code, label, discount_type, discount_value, max_discount_amount, min_booking_amount, is_active)
                VALUES
                ('WELCOME10','Welcome 10%','percent',10,1500,1000,1),
                ('FLAT500','Flat 500 off','flat',500,NULL,1500,1)");
        }

        try {
            $pdo->exec("INSERT INTO settings (`key`, `value`) VALUES
                ('vehicle_booking_module_enabled','1'),
                ('whatsapp_tpl_vehicle_booking_created','Hello {{name}}, your booking {{ref}} is received and currently {{status}}. Pickup: {{pickup}}'),
                ('whatsapp_tpl_vehicle_booking_assigned','Hello {{name}}, your booking {{ref}} is assigned. Driver will contact you shortly.'),
                ('whatsapp_tpl_vehicle_booking_started','Hello {{name}}, your trip {{ref}} is now on trip.'),
                ('whatsapp_tpl_vehicle_booking_completed','Hello {{name}}, your booking {{ref}} is completed. Thank you for riding with us.'),
                ('whatsapp_tpl_vehicle_booking_status_update','Hello {{name}}, your booking {{ref}} status is {{status}}.')
                ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)");
        } catch (\Throwable) {
        }
    },
    'down' => function (PDO $pdo): void {
        $pdo->exec('DROP TABLE IF EXISTS vehicle_maintenance_logs');
        $pdo->exec('DROP TABLE IF EXISTS booking_otp_verifications');
        $pdo->exec('DROP TABLE IF EXISTS vehicle_availability');
        $pdo->exec('DROP TABLE IF EXISTS booking_status_logs');
        $pdo->exec('DROP TABLE IF EXISTS vehicle_bookings');
        $pdo->exec('DROP TABLE IF EXISTS booking_coupons');
        $pdo->exec('DROP TABLE IF EXISTS pricing_rules');
        $pdo->exec('DROP TABLE IF EXISTS drivers');
